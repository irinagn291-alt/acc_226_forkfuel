# ForkFuel — Build Specification

> Batch 21AUG, app 04 of 30. This document is the complete brief for
...
**One-line positioning:** Fuel that matches the session.

| Field | Value |
| --- | --- |
| Product name | ForkFuel |
| Bundle identifier | `com.forkfuel.fuel` |
| Domain | https://forkfuel.pro |
| Contact URL | https://forkfuel.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone only, portrait |
| Interface style | Dark |
| Asset prefix | `ffl_` |
| User-Agent | `ForkFuel/1.0 (iOS; +https://forkfuel.pro)` |
...
7. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme ForkFuel -destination 'generic/platform=iOS' build`.
...
## 3. Uniqueness assignment for ForkFuel
...
| UI approach | **SwiftUI + UIKit mix via UIViewRepresentable** |
| Naming convention | **Domain-driven names** |
| File organization | **By module with subfolders** |
| Dependency strategy | **SPM remote x1 — swift-composable-architecture** |
| Design direction | **Athletic energy** |
| Typography | **Avenir Next** |
| Navigation pattern | **Navigation stack with push/pop** |
| AI art style | **Flat vector illustration** |
| Functional twist | **Training-day mode** |
| Persistence | **SwiftData** |
| Screen composition | see 3.6 |
| Scanner technology | **VisionKit DataScannerViewController** |
| Search endpoint | **/api/v2/search with fields list, page_size 12** |
| Day key representation | **Date normalised with Calendar.startOfDay** |

### 3.1 Architecture contract

swift-composable-architecture. One root `Feature` reducer composing child
reducers with `Scope`. State is a value type tree, all mutations happen in
`Reducer.body`, side effects returned as `Effect.run`. Dependencies injected
through `@Dependency` with a `DependencyKey` per service.
...
SwiftUI shell. At least three genuine `UIViewRepresentable`/
`UIViewControllerRepresentable` bridges: the barcode scanner, a `UIPickerView`
gram wheel, and a `UISlider`-based macro tuner. Wrapped views must round-trip
state through `Coordinator`.

### 3.3 Naming contract

Convention: Domain-driven names.

Examples to follow: `NutritionGateway`, `FuelIntakeAggregate`, `TrainingDayPolicy`, `PortionCalculator`

Apply it to types, files, properties, methods and asset names alike.

### 3.4 Dependency contract

`pointfreeco/swift-composable-architecture`, exact version pinned in project.yml.

### 3.5 Navigation contract

Single `NavigationStack` driven by a `StackState` path in TCA. No tabs, no modals for primary flow.
...
Strictly linear stack. Today, then Catalog which holds search and scan as
inner segments, then Detail, then Assign, then back to Today. Plan and Goals
push from Today. The log is inline in Today.
...
Technology: **VisionKit DataScannerViewController**

Tap a recognised barcode to select it; multiple codes may be visible.
...
- Open Food Facts search uses **/api/v2/search with fields list, page_size 12**.
- A day is represented internally as **Date normalised with Calendar.startOfDay**. Use this consistently in
  storage, in queries and in identifiers.
- The demo seed flag key is `ffl.demo.v1`.
- `NSCameraUsageDescription` is exactly: "ForkFuel reads barcodes to load food into your training-day plan."

---

## 4. Target file organization

Scheme: **By module with subfolders**

```
ForkFuel/
  Modules/Today/ Modules/Catalog/ Modules/Planner/ Modules/Goals/ Core/
  Assets.xcassets/
```
...
### 5.6 Assign
Choose a slot — Pre-Fuel, Refuel, Recovery, Top-Up — and choose
eaten-today or a future date. Top-Up is eaten-only and must be unavailable
...
| `surface` | `#2A2A2D` | Cards, rows, sheets |
| `ink` | `#F5F5F7` | Primary text and icons |
| `accent` | `#C6FF00` | Primary action, key figure, progress fill |
| `muted` | `#7A7A80` | Secondary text, dividers, disabled |
...
Avenir Next Demi Bold for headings, Regular for body, Heavy for the daily kcal figure.
...
Chosen technology: **SwiftData**

`@Model` classes: FuelEntry, FuelTarget, TrainingTag, WishItem. ModelContainer injected as a TCA dependency.
...
Set `User-Agent: ForkFuel/1.0 (iOS; +https://forkfuel.pro)` on every request. Never reuse another app's string.
Use the **/api/v2/search with fields list, page_size 12** search endpoint for this app.
...
INFOPLIST_KEY_NSCameraUsageDescription: ForkFuel reads barcodes to load food into your training-day plan.
...
## 12. Functional twist: Training-day mode

Each calendar day is tagged Training or Rest. Each tag carries its own macro
target set. Today's rings compare against the active tag's targets, and the
planner lets you pre-tag upcoming days.
...
given. Every name carries the `ffl_` prefix. No asset may be reused
...
```bash
sips -g pixelWidth -g pixelHeight -g hasAlpha ffl_AppIcon.png
...
```bash
sips -s format jpeg ffl_AppIcon.png --out tmp.jpg \
  && sips -s format png tmp.jpg --out ffl_AppIcon.png && rm tmp.jpg
```
...
| --- | --- | --- | --- | --- |
| 1 | `ffl_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `ffl_Splash` | 1290x2796 | allowed | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `ffl_Onboarding1` | 1024x1536 | allowed | Onboarding page 1 illustration: what the app is for. |
| 4 | `ffl_Onboarding2` | 1024x1536 | allowed | Onboarding page 2 illustration: scan and search. |
| 5 | `ffl_Onboarding3` | 1024x1536 | allowed | Onboarding page 3 illustration: setting daily targets. |
| 6 | `ffl_EmptyLog` | 1024x1024 | allowed | Empty state: nothing logged today yet. Calm and inviting, never sad. |
| 7 | `ffl_EmptySearch` | 1024x1024 | allowed | Empty state: search returned no products. |
| 8 | `ffl_EmptyPlan` | 1024x1024 | allowed | Empty state: no planned entries. |
| 9 | `ffl_EmptyWish` | 1024x1024 | allowed | Empty state: wish list has no items. |
| 10 | `ffl_SlotPreFuel` | 512x512 | allowed | Icon for the 'Pre-Fuel' slot. |
| 11 | `ffl_SlotRefuel` | 512x512 | allowed | Icon for the 'Refuel' slot. |
| 12 | `ffl_SlotRecovery` | 512x512 | allowed | Icon for the 'Recovery' slot. |
| 13 | `ffl_SlotTopUp` | 512x512 | allowed | Icon for the 'Top-Up' slot. |
| 14 | `ffl_MacroProtein` | 512x512 | allowed | Macro icon: protein. Must be visually distinct from carbs and fat at 24pt. |
| 15 | `ffl_MacroCarbs` | 512x512 | allowed | Macro icon: carbohydrates. |
| 16 | `ffl_MacroFat` | 512x512 | allowed | Macro icon: fat. |
| 17 | `ffl_ProductPlaceholder` | 600x600 | allowed | Fallback thumbnail shown when a product has no image. Used in every list row. |
| 18 | `ffl_CardBackdrop` | 1200x800 | allowed | Backdrop art for the product detail card. Low contrast so text stays readable. |
| 19 | `ffl_Texture` | 2048x2048 | allowed | Tiling background texture used at low opacity behind content. MUST tile with no visible seam. |
| 20 | `ffl_ControlFace` | 512x512 | allowed | Custom control artwork used for the primary interactive element. |
| 21 | `ffl_ScanOverlay` | 1024x1024 | required | Camera overlay art framing the barcode. The centre must be fully transparent. |
| 22 | `ffl_TwistHero` | 1024x1024 | allowed | Hero art for the 'Training-day mode' feature screen. |
| 23 | `ffl_SuccessMark` | 512x512 | allowed | Shown briefly when a food is logged successfully. |
| 24 | `ffl_HeaderDecor` | 1200x600 | allowed | Decorative header accent on the main screen. |

### Prompt per asset

**`ffl_AppIcon`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, the app's single emblem, centred, filling the canvas edge to edge
```

**`ffl_Splash`** — 1290x2796

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a vertical hero composition with a calm, uncluttered centre band
```

**`ffl_Onboarding1`** — 1024x1536

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a person or object representing discovering what is in packaged food
```

**`ffl_Onboarding2`** — 1024x1536

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a scanning or measuring motif showing a product being identified
```

**`ffl_Onboarding3`** — 1024x1536

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a goal or target motif showing daily progress being met
```

**`ffl_EmptyLog`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, an empty vessel, surface or container waiting to be filled
```

**`ffl_EmptySearch`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a search motif that has come back with nothing found
```

**`ffl_EmptyPlan`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, an empty schedule, grid or horizon with nothing scheduled
```

**`ffl_EmptyWish`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, an empty basket, list or shelf
```

**`ffl_SlotPreFuel`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a morning motif appropriate to the theme
```

**`ffl_SlotRefuel`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a midday motif appropriate to the theme
```

**`ffl_SlotRecovery`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, an evening motif appropriate to the theme
```

**`ffl_SlotTopUp`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a small extra or in-between motif appropriate to the theme
```

**`ffl_MacroProtein`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a symbol standing for protein, rendered as a single clear emblem
```

**`ffl_MacroCarbs`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a symbol standing for carbohydrate, rendered as a single clear emblem
```

**`ffl_MacroFat`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a symbol standing for dietary fat, rendered as a single clear emblem
```

**`ffl_ProductPlaceholder`** — 600x600

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a generic packaged grocery item with no readable branding
```

**`ffl_CardBackdrop`** — 1200x800

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, an abstract backdrop suitable for sitting behind a product card
```

**`ffl_Texture`** — 2048x2048

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a seamless repeating surface pattern
```

**`ffl_ControlFace`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, the face of a single physical control such as a dial, key or slider handle
```

**`ffl_ScanOverlay`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a framing reticle or targeting bracket, open in the middle
```

**`ffl_TwistHero`** — 1024x1024

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, an emblem representing this app's signature feature
```

**`ffl_SuccessMark`** — 512x512

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a confirmation mark or celebratory emblem
```

**`ffl_HeaderDecor`** — 1200x600

```
flat vector illustration, corporate sports style, clean geometric lines, bold lime accent on charcoal, no gradients, no texture, a wide decorative band or ornament
```
...
- `ffl_Texture` must tile seamlessly. Verify by placing four copies
  edge to edge.
- `ffl_ScanOverlay` needs a genuinely transparent centre.
...
| Rice Cakes | `5411188110316` | 387 | 8.0 | 81.5 | 2.8 |
| Kefir | `4607091382010` | 41 | 3.3 | 4.1 | 1.0 |
| Couscous | `3038350208002` | 376 | 12.8 | 77.4 | 0.6 |
| Rolled Oats | `3013710000201` | 379 | 13.2 | 67.7 | 6.5 |
...
| Role | Label in ForkFuel | Planned | Eaten |
| --- | --- | --- | --- |
| Morning meal | Pre-Fuel | yes | yes |
| Midday meal | Refuel | yes | yes |
| Evening meal | Recovery | yes | yes |
| Snack | Top-Up | no | yes |
...
Add a unit test target `ForkFuelTests` covering at minimum:
...
- [ ] `xcodebuild -scheme ForkFuel -destination 'generic/platform=iOS' build` succeeds.
...
- [ ] All four slots accept entries; Top-Up rejects future dates.
...
**Uniqueness**
- [ ] Architecture matches **The Composable Architecture (TCA)** with no leakage across layers.
- [ ] UI approach matches **SwiftUI + UIKit mix via UIViewRepresentable**.
- [ ] Navigation matches **Navigation stack with push/pop**.
- [ ] Screen composition follows section 3.6, not the generic list in section 5.
- [ ] Scanner uses **VisionKit DataScannerViewController**.
- [ ] Day key is **Date normalised with Calendar.startOfDay**.
...
- [ ] Demo seed runs only on Simulator, once, behind `ffl.demo.v1`.
...
**Assets**
- [ ] All 24 images generated in the **Flat vector illustration** style, prefixed `ffl_`.
